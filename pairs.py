...
from constants import CUSTOMERS_TABLE
...
def pairsAreNotValid(customers):
  for i in range(0, len(customers) - len(customers) % 2, 2):
    ...
    if previousPairIDs and pairID in previousPairIDs:
      print(
          f"NOT UNIQUE: {customers[i+1]['fields']['First Name']} is in {customers[i]['fields']['First Name']}'s list"
      )
      return True
    ...
    if isNotValidGenderPair(customers, i, i + 1):
      print(
          f"GENDER MISMATCH: {customers[i+1]['fields']['First Name']} isn't in {customers[i]['fields']['First Name']}'s list"
      )
      return True
  ...
  if len(customers) % 2 == 1:
    ...
    if previousPairIDs and (id1 in previousPairIDs or id2 in previousPairIDs):
      print(
          f"NOT UNIQUE: {customers[-3]['fields']['First Name']} or {customers[-2]['fields']['First Name']} is in {customers[-1]['fields']['First Name']}'s list"
      )
      return True

    # CHECK IF MATCHES GENDER PREFS
    if isNotValidGenderPair(customers, -1, -2) or isNotValidGenderPair(
        customers, -1, -3
    ):
      print(
          f"GENDER MISMATCH: {customers[-3]['fields']['First Name']} or {customers[-2]['fields']['First Name']} isn't in {customers[-1]['fields']['First Name']}'s list"
      )
      return True
